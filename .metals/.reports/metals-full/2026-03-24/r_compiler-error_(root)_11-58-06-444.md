...
case class GameState(board: Board, turn: Int, random: MyRandom) {

  val openCoords = GameState.getOpenCoords(B@@)

  def randomMove(): = GameState.randomMove()
  def play()  
}

object GameState {
  def randomMove(openCoords: List[Coord2D], rand: MyRandom)
}
